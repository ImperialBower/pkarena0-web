...
| Component | Status |
|---|---|
| Load `BotProfile` YAML (embedded `data/bots/standard.yaml`) instead of only `default_profiles()` | Done (`standard_profiles()`, `src/lib.rs`) |
| YAML validation + code-parity gate (`make validate-bots`) | Done (`bot_bundle_fixture` tests; Makefile hook) |
| Position awareness: `Playbook` attached to all archetypes (today 3 of 9) | Planned (Phase 2) |
| Difficulty selector UI → profile bundle (weak / standard / strong) | Planned (Phase 3) |
| Strong bundle consumes EPIC-36 `decision:` knobs + EPIC-47 adaptation | Planned (post upstream EPIC-36) |
| Arena mode chips/100 report (seeded, exportable) | Planned (Phase 3) |
...
| File | Role |
|---|---|
| `data/bots/standard.yaml` (done); `{weak,strong}.yaml` (Phase 3) | lineup bundles |
| `src/lib.rs` `standard_profiles()` / `BotBundle` | YAML pool construction |
| `bot_bundle_fixture` tests + `make validate-bots` | validation (no bin — `cdylib` crate) |
| `www/` setup screen | difficulty selector (Phase 3) |
