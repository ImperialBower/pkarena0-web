# EPIC-54: Player-Mode Table Setup — Opponent Count & Bot Selection

> **Provenance:** Drafted 2026-07-15 on the `EPIC-50` branch (commit `89205c7`)
> as EPIC-51, then superseded on `main` by
> [EPIC-51 `strengthen()` Isolation & Re-scope](EPIC-51_Strengthen_Rescope.md).
> Salvaged into `docs/` and **renumbered EPIC-51 → EPIC-54** (next free number)
> on 2026-07-18. Code-line references reflect the `main` snapshot at draft time
> (`89205c7` / `55a862c`, pre-EPIC-46 merge) and may have shifted — re-verify
> anchors before implementing. The `89205c7` seeded-equivalence baseline below
> means "today's default `init_game` behavior" as of that draft.
...
**Depends on:** nothing unmerged — builds directly on `main`.
**Related:** [EPIC-49](EPIC-49_Bot_Lineup_Difficulty.md)
...
- [ ] 3c. Add EPIC-54 row to `docs/EPICS.md`.
